...
#############################
# Foundry utilities (local or Docker fallback)
#############################

# Define run_cast() to either use local 'cast' or a Docker-based fallback
if command -v cast >/dev/null 2>&1; then
  run_cast() { cast "$@"; }
else
  echo "Foundry 'cast' not found locally; using Docker fallback."
  run_cast() {
    docker exec nitro-dev cast "$@"
  }
fi

if command -v run_cast >/dev/null 2>&1; then
  echo "Setting chain owner to pre-funded dev account..."
  run_cast send 0x00000000000000000000000000000000000000FF "becomeChainOwner()" \
    --private-key $PRIVATE_KEY \
    --rpc-url $RPC || echo "Warning: becomeChainOwner failed"
  ...
  run_cast send -r $RPC --private-key $PRIVATE_KEY 0x0000000000000000000000000000000000000070 'setL1PricePerUnit(uint256)' 0x0 || echo "Warning: setL1PricePerUnit failed"

  echo "Deploying the CREATE2 factory"
  run_cast send --rpc-url $RPC --private-key $PRIVATE_KEY --value "1 ether" 0x3fab184622dc19b6109349b94811493bf2a45362 || echo "Warning: funding factory failed"
  run_cast publish --rpc-url $RPC  || echo "Warning: publish factory failed"
  if [ "$(run_cast code -r $RPC $CREATE2_FACTORY 2>/dev/null)" == "0x" ]; then
    echo "Warning: CREATE2 factory not deployed"
  fi

  echo "Deploying Cache Manager contract..."
  deploy_output=$(run_cast send --private-key $PRIVATE_KEY \
    --rpc-url $RPC \
  ...
  if [[ -z "$contract_address" ]]; then
    echo "Warning: Failed to extract Cache Manager address; skipping registration"
  else
    ...
    registration_output=$(run_cast send --private-key $PRIVATE_KEY \
      --rpc-url $RPC \
      0x0000000000000000000000000000000000000070 \
      "addWasmCacheManager(address)" "$contract_address")
    if [[ "$registration_output" == *"error"* ]]; then
      echo "Warning: Cache Manager registration failed"
    else
  ...
  deployer_address=$(run_cast create2 --init-code $deployer_code 2>/dev/null)
  run_cast send --private-key $PRIVATE_KEY --rpc-url $RPC \
      $CREATE2_FACTORY "$SALT$deployer_code" || echo "Warning: StylusDeployer send failed"
  if [ "$(run_cast code -r $RPC $deployer_address 2>/dev/null)" == "0x" ]; then
    echo "Warning: StylusDeployer not deployed"
  else
    echo "StylusDeployer deployed at address: $deployer_address"
  fi

  echo "Funding accounts with 1 ETH each..."
  run_cast send --private-key $PRIVATE_KEY --rpc-url $RPC --value "1 ether" 0xDD09b55496EaA3cFAe23137ABDeA52a9a979B70e || true
  run_cast send --private-key $PRIVATE_KEY --rpc-url $RPC --value "1 ether" 0xE9cB1563bE49002383D08386ee287aF7BAD08c3b || true
  run_cast send --private-key $PRIVATE_KEY --rpc-url $RPC --value "1 ether" 0x838d568Ffb16BC74083e88fd769df85E8d3afcE6 || true
  run_cast send --private-key $PRIVATE_KEY --rpc-url $RPC --value "1 ether" 0xA491d1134388c78AeEDf6b1Ca3F21657015Ff8E1 || true
  echo "(Optional) Accounts funded with 1 ETH each"
fi
